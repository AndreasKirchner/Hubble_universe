...
@inline function system_of_equation3(u::M,T,μ,wal::WaleckaModel{N}) where {M<:AbstractArray,N}
    ...
    n_lie32p=polylog_exp_norm(3/2,nump,T)
    n_lie52p=polylog_exp_norm(5/2,nump,T)
    @show nump/T   

    n_lie32m=polylog_exp_norm(3/2,numm,T)
    n_lie52m=polylog_exp_norm(5/2,numm,T)
...
function gapequation3(T,mu,wal::WaleckaModel{N}) where {N}
...
function thermodynamic(T::A,mu::B,wal::WaleckaModel{L}) where {A,B,L}
    ...
    n_lie32p=polylog_exp_norm(3/2,nump,T)
    n_lie52p=polylog_exp_norm(5/2,nump,T)
    n_lie12p=polylog_exp_norm(1/2,nump,T)
    

    n_lie32m=polylog_exp_norm(3/2,numm,T)
    n_lie52m=polylog_exp_norm(5/2,numm,T)
    n_lie12m=polylog_exp_norm(1/2,numm,T)
