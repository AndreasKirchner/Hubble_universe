...
using Bessels:besselk,besselk0,besselk1
...
#read in resonaces
function HadronResonaceGas(;name_file="EquationofState/particles.data",Maxmass=2.1,Minmass=0.05,condition=x->true)
...
#read in resonances
function HadronResonaceGasNew(;name_file="EquationofState/PDG2016Plus_massorder.dat",Maxmass=2.1,Minmass=0.05,condition=x->true)
    data     =readdlm(name_file,comment_char='#',comments=true)
...
TBW
"""
function thermodynamic(beta,alpha,x::HadronResonaceGasNew{L})  where {L}
    ...
    for i in eachindex(x) 
        ...
        if QB==0
            ...
            for n in 1:4
                ...
                besslk2n=besslk0n+2/(n*reducemass)*besslk1n
                besslk3n=besselk(3,n*reducemass)
                besslk4n=besselk(4,n*reducemass)
                expn=exp(n* alpha* QB )

                pressure+=degeneracy*(-1)^(n+1)*expn*m^2*besslk2n/(2*n^2*pi^2*beta)
                p10+=degeneracy*(-1)^n*expn*m^2*(n*reducemass*besslk1n+2*besslk2n+n*reducemass*besslk3n)/(4*n^2*p^2*beta^2)
                p01+=-degeneracy*(-1)^n*expn*m^2*besslk2n/(2*n*pi^2*beta)
                p20+=-degeneracy*(-1)^n*expn*m^2*(n^2*reducemass^2*besslk0n+4*n*reducemass*besslk1n+(8+2*n^2*reducemass^2)*besslk2n+4*n*reducemass*besslk3n+n^2*reducemass^2*besslk4n)/(8*n^2*pi^2*beta^3)
                p02+=-degeneracy*(-1)^n*expn*m^2*besslk2n/(2*pi^2*beta)
                p11+=degeneracy*(-1)^n*expn*m^2*(n*reducemass*besslk1n+2*besslk2n+n*reducemass*besslk3n)/(4*n*pi^2*beta^2)              
            end    
        else
            ## loop over barions 
            for n in 1:3
                ...
                pressure+=degeneracy*expn*m^2*besslk2n/(2*n^2*pi^2*beta)
                p10+=-degeneracy*expn*m^2*(n*reducemass*besslk1n+2*besslk2n+n*reducemass*besslk3n)/(4*n^2*p^2*beta^2)
                p01+=0*degeneracy*expn*m^2*besslk2n/(2*n*pi^2*beta)
                p20+=degeneracy*expn*m^2*(n^2*reducemass^2*besslk0n+4*n*reducemass*besslk1n+(8+2*n^2*reducemass^2)*besslk2n+4*n*reducemass*besslk3n+n^2*reducemass^2*besslk4n)/(8*n^2*pi^2*beta^3)
                p02+=0*degeneracy*expn*m^2*besslk2n/(2*pi^2*beta)
                p11+=0*degeneracy*expn*m^2*(n*reducemass*besslk1n+2*besslk2n+n*reducemass*besslk3n)/(4*n*pi^2*beta^2) 
            end
